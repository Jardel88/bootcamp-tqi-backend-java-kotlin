...
/*
    ...
    de construção da representação final do objeto. Isso ajuda a melhorar a legibilidade e a flexibilidade do código,
    especialmente quando você precisa criar objetos com muitos parâmetros ou configurações diferentes.

    Neste desafio, buscando soluções para um problema de negócios em uma plataforma de e-commerce, é necessário melhorar
    a forma como os pedidos personalizados são tratados. Você tem a opção de implementar a solução utilizando o padrão
    Builder para criar pedidos de forma mais eficiente e organizada, ou seguir uma abordagem alternativa sem a necessidade
    ...
    Nota:
    O padrão Builder é uma abordagem valiosa quando se trata de criar objetos complexos com diferentes configurações.
    No entanto, deve-se considerar a relação entre a complexidade do problema e a necessidade de aplicar o padrão,
    especialmente em cenários mais simples. A decisão de usar ou não o padrão Builder dependerá das necessidades específicas
    do projeto e das características do sistema em que está sendo aplicado.
...
class CustomOrder private constructor(
    ...
    class Builder {
        ...
        fun build(): CustomOrder {
            ...
            return CustomOrder(customerName, products, total = totalP, deliveryAddress)
        }
    }
...
fun main() {
    ...
    println("Digite o número de produtos:")
    val numProducts = readLine()?.toIntOrNull()
    for (i in 1..numProducts!!) {
        println("digite o nome do produto ${i}:")
        val productName = readLine() ?: ""
        println("digite o preço do(a) ${productName}")
        val productPrice = readLine()?.toDoubleOrNull()
        println("digite a quantidade de ${productName}:")
        val productQuantity = readLine()?.toIntOrNull()
